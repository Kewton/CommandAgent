...
| Phase35 | setup/profile/dev-server/readiness contract connection | Setup/profile/dev-server rows disagree with current recovery-task output, and the issue is not limited to Next.js. | `focused-nextjs-dependency-setup`, `focused-nextjs-dev-server-port-conflict`, `focused-nextjs-endpoint-smoke`, `focused-nextjs-manifest-repair`, `focused-nextjs-route-integration`, `focused-phase26-setup-node-readiness`, `focused-setup-manifest-invalid`. | Setup/profile/dev-server/readiness rows pass recheck or have explicit row-level accepted disposition; current broad sign-off no longer reports disconnected setup/profile rows. |
