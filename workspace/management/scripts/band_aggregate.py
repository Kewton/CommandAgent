...
def assert_cli_invariants(records: list[CliRunRecord]) -> int:
    ...
    assert all(
        record.api_input_tokens is not None
        and record.api_output_tokens is not None
...
def cli_band_status(set_id: str) -> str:
    ...
    if set_id == CLI_LUNA_SETS[0]:
        return (
            "machine BLOCKED — provider rejected function tools under "
            "the omitted reasoning_effort default before C checks"
        )
    if set_id == CLI_LUNA_SETS[1]:
        return (
            "machine BLOCKED — explicit-only request fixture and live unset confirm "
            "client omission; provider omitted default still rejected"
        )
    if set_id == CLI_LUNA_SETS[2]:
        return (
            "OpenAI Luna F-2a-3 text-protocol window — endpoint crossed; "
            "6/6 stopped on text tool-call shape before C checks"
        )
    if set_id == CLI_LUNA_SETS[3]:
        return (
            "OpenAI Luna F-2a-5 self-recording window — 6/6 parse evidence; "
            "anatomy a=4, b=2, c=0 before C checks"
        )
    if set_id == CLI_LUNA_SETS[4]:
        return (
            "OpenAI Luna F-2a-6 dialect-calibrated window — repair_applied 9 "
            "(first JSON 1 / closing tag 8); residual b=4 + empty=2; "
            "C checks 0/6"
        )
    if set_id == CLI_LUNA_SETS[5]:
        return (
            "OpenAI Luna F-0b Responses/native window — 112/112 provider turns "
            "and 115 native tool calls; C checks 5/6; C3 pass 2 / fail 1 / "
            "claims_absent 2; full 0/6"
        )
...
def build_cli_summary(
    ...
    lines = [
        ...
        f"- Source sets: `{len(scanned_sets)}` (local 1 + elevated 4 + pack arms 3 + Luna arm {len(CLI_LUNA_SETS)} + directive source 1)",
        f"- Observed runs: `{len(records)}`",
        ...
        "- Luna interpretation: n=36 total. The 001/002 windows are machine BLOCKED 12/12 before generation (reasoning default × Chat Completions tools). The 003-005 text windows crossed the endpoint but stopped before C checks; the 005 dialect-calibrated window emitted repair_applied 9/9 yet retained residual b=4 or empty response=2. The 006 Responses/native window recorded 112/112 native-enabled provider turns and 115 function calls, reached C checks 5/6, and produced the first Luna C3 distribution: pass 2 / violation 1 / claims_absent 2. Full remains 0/36; two C3 matches show that the testimony wall can move with model class and the correct endpoint, while C1 placeholder binding, runtime compatibility, one C3 mismatch, and one assurance projection gap remain. Response model/service tier are observed while system_fingerprint is null.",
