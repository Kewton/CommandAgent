...
def build_cli_summary(
    ...
    lines = [
        ...
        f"- Source sets: `{len(scanned_sets)}` (local 1 + elevated 4 + pack arms 3 + Luna arm 5 + directive source 1)",
        f"- Observed runs: `{len(records)}`",
        ...
        "- Luna interpretation: n=30 total. The 001/002 windows are machine BLOCKED 12/12 before generation (reasoning default × Chat Completions tools). The 003/004 windows explicitly select the established text protocol: 74/74 OpenAI turns crossed the endpoint with tools=0, then 12/12 stopped on model-produced XML/JSON tool-call shape before C checks. The 005 dialect-calibrated window emitted repair_applied 9/9 (first JSON 1 / closing tag 8) with evidence, but all 6 runs still stopped before C checks on residual b=4 or empty response=2. Across 003-005, 155/155 OpenAI turns crossed the endpoint with tools=0. C3 testimony behavior remains unobserved; response model/service tier are observed while system_fingerprint is null. F-0b native tools is the next adjudication candidate.",
