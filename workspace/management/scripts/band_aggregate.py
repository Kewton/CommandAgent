...
def build_cli_summary(
    ...
    lines = [
        ...
        f"- Source sets: `{len(scanned_sets)}` (local 1 + elevated 4 + pack arms 3 + Luna arm 3 + directive source 1)",
        f"- Observed runs: `{len(records)}`",
        ...
        "- Luna interpretation: n=18 total. The 001/002 windows are machine BLOCKED 12/12 before generation (reasoning default × Chat Completions tools). The 003 window explicitly selects the established text protocol: 24/24 OpenAI turns crossed the endpoint with tools=0, then 6/6 stopped on model-produced XML/JSON tool-call shape before C checks. C3 testimony behavior remains unobserved; response model/service tier are observed while system_fingerprint is null.",
