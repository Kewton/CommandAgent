 ...
 */
function readJsonArray<T>(filePath: string, defaultVal: T[]): [T[], null] | [null, ApiResponse<unknown>["error"]] {
  try {
    ensureDataDir();
...
export function loadProjects(): { data: Project[] | null; error: ApiResponse<unknown>["error"] | null } {
  const [data, err] = readJsonArray<Project>(PROJECTS_FILE, []);
  return { data, error: err };
}

/** Persist projects to disk. */
export function saveProjects(projects: Project[]): { ok: boolean; error: ApiResponse<unknown>["error"] | null } {
...
export function loadTasks(): { data: Task[] | null; error: ApiResponse<unknown>["error"] | null } {
  const [data, err] = readJsonArray<Task>(TASKS_FILE, []);
  return { data, error: err };
}

/** Persist tasks to disk. */
export function saveTasks(tasks: Task[]): { ok: boolean; error: ApiResponse<unknown>["error"] | null } {
...
export function createProject(body: CreateProjectBody): Promise<{ data: Project | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
    // Validate
...
export function getProjects(): Promise<{ data: Project[] | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
...
export function createTask(body: CreateTaskBody): Promise<{ data: Task | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
    // Validate
...
export function getTasks(filter?: TaskFilterParams): Promise<{ data: Task[] | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
...
export function updateTaskStatus(taskId: string, status: TaskStatus): Promise<{ data: Task | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
...
export function assignTask(taskId: string, assignee: Member | null): Promise<{ data: Task | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
    if (assignee !== null) {
...
export function deleteTask(taskId: string): Promise<{ data: { id: string; deleted: boolean } | null; error: ApiResponse<unknown>["error"] | null }> {
  return withMutex(() => {
    const { data: tasks, error: loadErr } = loadTasks();
    if (loadErr || !tasks) {
      return Promise.resolve({ data: null, error: loadErr ?? { code: "IO_ERROR", message: "Failed to load tasks" } });
