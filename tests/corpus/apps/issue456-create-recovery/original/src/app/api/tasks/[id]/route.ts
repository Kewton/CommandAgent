...
import { UpdateTaskBody, TaskStatus } from "@/lib/types";
...
export async function PATCH(
  ...
  try {
    ...
    const { status, assigneeId } = body as Partial<UpdateTaskBody>;
        ...
        return NextResponse.json(
          {
            ...
            details: "assigneeId must be a string or null.",
          },
          { status: 400 }
        );
      }

      const result = await assignTask(id, assigneeId as string | null);
      if (result.ok) {
        return NextResponse.json(
          { item: result.item },
          { status: 200 }
    ...
    return NextResponse.json(
      {
        ...
        details: "Provide 'status' and/or 'assigneeId' in the request body.",
      },
      { status: 400 }
    );
  } catch (err) {
