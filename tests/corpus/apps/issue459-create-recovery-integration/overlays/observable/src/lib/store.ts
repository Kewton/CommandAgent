...
function atomicWriteJson(filePath: string, data: unknown): void {
  ...
  // Keep atomic replacement while making the two stable output sinks explicit.
  // This application keeps process.cwd() fixed at its project root.
  if (filePath === PROJECTS_FILE) fs.renameSync(tmpPath, "data/projects.json");
  else if (filePath === TASKS_FILE) fs.renameSync(tmpPath, "data/tasks.json");
...
// Fixture team: the original goal asks for assignable team members.
export function getMembers(): Member[] {
  return [
    { id: "member-1", name: "Aki", role: "designer" },
    { id: "member-2", name: "Ren", role: "developer" },
  ];
}
