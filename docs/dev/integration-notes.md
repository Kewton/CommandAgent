...
| D-3c PM router + boundary dialogue shell | **QUEUED; design review next** | Design the request-confirmation, unattended-execution, acceptance, and failure gates from the Phase C handoff before authorizing product REPL changes. |
| P-3 user-loop proof | **QUEUED; after D-3c** | After the boundary shell is admitted, measure one end-to-end user loop from route proposal and human confirmation through execution, full acceptance-sheet review, and a human-selected next action. Do not substitute a scripted self-demo. |
