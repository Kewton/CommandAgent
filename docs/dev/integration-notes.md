...
| D-3c interaction follow-ups | **PACK SUPPLY CONTRACT v0.1 FIXED; IMPLEMENTATION QUEUED** | Pack selection now has the fixed `PackSource` values `admitted | repository | local`; pinned repository/local packs may be explicitly selected with unapproved/band-unmeasured disclosure, and extension-root wins same-identity resolution. E-13a/E-14/E-15/E-16 implement the CLI, REPL, GUI, and supply paths. Presentation polish and multiple requests per REPL session remain separate. Signed/remote supply, publisher trust, and revocation remain Phase G rather than being implied by unsigned local supply. |
