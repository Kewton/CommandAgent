...
| D-3c interaction follow-ups | **QUEUED** | Polish the boundary presentation, support multiple requests in one REPL session, expose admitted-pack selection instead of display-only compatibility, and wire interactive Gate 4 next-action selection. These do not change the completed four-gate safety boundary. |
