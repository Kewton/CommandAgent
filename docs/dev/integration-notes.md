...
| D-3c interaction follow-ups | **QUEUED** | Polish the boundary presentation, support multiple requests in one REPL session, and expose admitted-pack selection instead of display-only compatibility. D-3d v0 wires the Gate 4 human-directive action only; other interactive next actions remain separate. |
...
| F-2 Luna CLI comparison | **QUEUED; suite design** | Define `cli-create-luna` with the same CLI create scenarios, `executor=gpt-5.6-luna`, and the local `qwen3.6:27b-coding-nvfp4` planner. Keep the mixed-provider shape identical to the Gemma cloud arm and pin returned OpenAI model/fingerprint metadata as a configuration-identity probe. |
| F-0b OpenAI Responses native tools | **QUEUED** | Add the formal native-tool path through the Responses API. Until then, Chat Completions models whose endpoint rejects function tools must declare `tool_protocol=text`; do not add model-name sniffing or a provider-call bypass. |
