...
聴衆なしで台本を通読し、各幕の指定ファイルを実際に開いて確認した。実測合計は **8分40秒**。
第2幕のE2表（26行）と第5幕のE-A〜E-D詳細が最も時間を要したため、10分枠では表を全件スクロールせず、冒頭と末尾を示して件数を読み上げる調整とした。第4幕は拒否記録の該当行へ直行する。停止・再試行はなし。
