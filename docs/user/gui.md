...
3. Select **Confirm and delegate to CLI**. The server starts the existing
   ...
   confirmed contract cannot be invalidated by an in-flight edit.
...
Gate 3/4, the read-only launch identity, CLOSED-to-compose recovery and a
second terminal run, the first session event stream and its SHA-256, and an
API log:
