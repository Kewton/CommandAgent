...
| `409 trial_workspace_conflict` | If the message names a session, use the displayed reconnect link. If it says recovery is required, inspect that session's events and complete the existing CLI/runtime recovery procedure before reconnecting. Do not delete `.anvil/` state to bypass the lease. |
