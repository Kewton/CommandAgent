...
| D-2c | 計画確定チョークポイントの一本化 | 本コミット | 複数扉一門番病（時間軸版・4例目）の恒久修正。変異→正規化→lintをチョークポイントで構造強制 | `finalize_step_plan_for_execution`へ修復・sanitize・lintを集約し、planner本番経路の直接lint呼び出しを字句guardrailで1箇所に固定。既存fixtureでイベント形・verdict不変を確認。作業時間概算: 約30分 | **complete** |
