...
`http://127.0.0.1:4173/` を開きます。**概要** は最初に、製品を **Goal →
実行前確認 → 計画・実装 → 検証・修復 → 検証済み結果または正直な失敗**として
説明します。Gate、profile、pack、assurance を平易に定義し、4 つの拡張
レイヤーを要約します。能力マップ／band と repository run 一覧は重複表示せず、
担当する **計測** と **リポジトリ実行記録**へリンクします。

常に表示される **FIRST USE / はじめに** は、execution root、extension root、
CLI バイナリ、Trial アクセスの 4 項目を再確認し、サンプル目標を提示します。
後半の **現在の状態** は実データの Trial readiness、active session、秘匿された
extension-root 状態だけを表示します。読み込み中や取得失敗を green の成功表示には
しません。
...
**確認して CLI を実行** の後、ブラウザは
`/try/status/?session=<id>` へ移動します。意図的に 2 つのものを分けて表示します。
**実行状態**（`running` と、`events.jsonl`
から再構成した phase 一覧）と **監視の健全性**（`接続中` / `不安定` / `切断`）
です。ブラウザがサーバーを見失っても CLI は動き続けます。status route を直接
再読み込みするか、URL のセッション ID で **監視を再接続** してください。
...
終端に到達すると、ブラウザは `/try/history/detail/?session=<id>` へ移動します。
**結果詳細**では、判定・保証水準・プロセス状態が 3 つの別々の事実として表示され、
`summary.md`、イベント末尾、確認記録を開けます。任意で追加の依頼を入力でき、
それも独自の確認を通ります。

![Gate 3/4 の結果](../../assets/tutorial/gui-07-result.png)

GUI から起動したセッションは `/try/history/` の **実行履歴** に分けて表示します。
compact row は identity の要約だけを持ち、active session は実行状況、terminal
session は結果詳細へリンクします。診断と artifact は **結果詳細**が所有します。
