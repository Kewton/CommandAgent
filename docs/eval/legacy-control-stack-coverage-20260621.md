...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Task contract core | `task_contract.rs`, `task_contract_core.rs`, `task_contract_taxonomy.rs`, `task_contract_display.rs` | Holds task purpose, kind, artifacts, constraints, and expected completion evidence. | Step runner plan schema, profiles, artifact graph | Partial | Partial | Step/ultra plans carry instructions, required artifacts, verifiers, and profile hints. | Add request-to-contract inference, contract admission, behavior projection, and lifecycle state as explicit planning/recovery contracts. |
...
| Objective and behavior contract projection | `objective_contract_projection.rs`, `behavior_contract_projection_e2e_tests.rs`, `required_behavior.rs`, `behavior_delta_obligation.rs`, `contract_bound_generation.rs`, `contract_generation_expectations.rs` | Projects user-visible required behavior into obligations and completion checks. | Plan prompt, plan lint, profile verification | Partial | Partial | Some required artifacts and profile obligations are projected into steps. | Add behavior-obligation schema before adding more ad hoc profile rules. |
| Artifact role taxonomy | `task_contract_artifact_contract.rs`, `task_contract_artifact_predicates.rs`, `task_contract_artifact_intent.rs`, `artifact_target_alignment.rs` | Classifies artifacts as setup, implementation, test, docs, data, route/integration targets. | ArtifactGraph / profiles | Partial | Partial | Path-based `ArtifactRole` and profile-specific role checks exist. | Build unified role SSOT across plan lint, profile verification, verifier repair, and recovery admission. |
| Task workspace scope | `task_workspace_scope.rs`, `workspace_access.rs`, `workspace_candidates.rs`, `workspace_walk.rs`, `workspace_paths.rs` | Decides which subtree this task may claim ownership over. | Safety/path confinement, recovery contract label | Partial | Adopt | `workspace_scope` is rendered as a recovery label derived from target role. | Add explicit/greenfield/single-project/ambiguous-parent scope detection, monorepo subtree handling, and scope-aware target admission. |
| Artifact ownership | `artifact_ownership.rs`, `owned_test_projection.rs`, `artifact_state_projection.rs` | Distinguishes owned artifacts from candidate-only or out-of-scope files. | ArtifactGraph / recovery contract | Partial | Adopt | `artifact_ownership` describes graph source/lifecycle for admitted target. | Add `Owned`/`CandidateOnly`/`OutOfScope`, edited-this-session, scaffold-delta, and verifier-owned signals. |
| Artifact ledger | `artifact_ledger.rs`, `artifact_ledger_state.rs`, `repo_edit_observation.rs`, `post_tool_reconciliation.rs` | Records per-turn artifact observations, edits, scaffold deltas, and verifier observations. | Minimal loop result / step runner evidence | Partial | Adopt | `Write` and `Edit` tool records retain exact target paths; repair changed-file markers now render paths instead of tool names; a bounded artifact-ledger summary can classify tool targets by role and ownership. | Add verifier observations, scaffold deltas, post-tool reconciliation, and eval report projection. |
| Completion evidence | `completion_evidence.rs`, `success.rs`, `completion_probe_gate.rs`, `objective_evidence.rs`, `evidence_observation.rs` | Converts actual tool/build/doc/data observations into completion authority. | Step verifier, final-answer guard, eval | Partial | Adopt | Verifiers and profile checks produce pass/fail, but completion evidence is not a unified runtime authority. | Add typed evidence set for repo edits, verifier exit zero, docs/data/report completeness, and command observations. |
| Evidence binding | `evidence_binding.rs`, `evidence_runner.rs`, `evidence_binding` adapters | Checks whether a deliverable can bind to its evidence runner before execution. | Verifier/profile/setup | Missing | Adopt | Dependency/setup failures are partially separated, but runner binding is not a first-class job. | Add binding failures for runner manifest, docs section, schema output, source citation, and file layout. |
| Deliverable obligation audit | `deliverable_obligation_audit.rs`, `task_contract_deliverable_projection.rs`, `task_contract_deliverable_lifecycle.rs`, `deliverable_freshness.rs` | Audits required deliverables, freshness, and non-coding artifact obligations. | Plan lint / eval / profile | Missing | Adopt | Required artifacts exist, but freshness and deliverable lifecycle are not centrally audited. | Add read-only deliverable obligation projection and freshness checks. |
...
| Recovery owner / dispatch gate | `active_job_arbiter.rs`, `repair_job_dispatch.rs`, `artifact_recovery_flow.rs` | Prevents multiple recovery systems from acting at once. | Recovery orchestration | Missing | Adopt | No dedicated owner gate; priority is only metadata/order. | Add one active owner, admitted next action, and disallowed competing recovery paths. |
...
| Repair action space | `repair_action.rs`, `repair_action_space.rs`, `repair_authority.rs`, `repair_plan.rs`, `repair_plan_admission.rs` | Validates whether a selected action is admissible for selected target/authority. | Recovery orchestration | Partial | Adopt | `RecoveryActionKind`, `ToolPolicyProjection`, and `allowed_change_kind` exist. | Add `RepairActionPlan` status, target role/path match, allowed tool category, rejection reason, authority, and lifecycle. |
...
| Repair job state machine | `repair_job.rs`, `repair_job_dispatch.rs`, `repair_lifecycle.rs`, `repair_progress.rs`, `verifier_repair_pass_flow.rs` | Tracks verifier repair attempts, rerun outcome, repeated signatures, target exhaustion, and safe stop. | Repair loop / recovery task | Partial | Adopt | Bounded repair packet exists; recovery task renders structured fields. | Add persistent repair job state, repeated signature thresholds, rerun outcome classification, target attempt buckets, and safe-stop report. |
| Repair attempt ledger | `repair_attempt_outcome.rs`, `repair_progress.rs`, `repair_job.rs` | Records attempt outcome by target and failure cluster. | Recovery task / repair loop | Partial | Adopt | Prior attempts are string summaries; correction exhaustion can be included. | Add buckets for duplicate/noop/worsened/improved/unsafe, exact target path, and exhausted target list. |
...
| Verifier diagnostic assessment | `verifier_diagnostic_flow.rs`, `verifier_diagnostic_attempt.rs`, `verifier_diagnostic_prompt.rs`, `verifier_assessment_parser.rs`, `verifier_weak_reason.rs`, `verifier_weak_repair_target.rs` | Parses verifier failure and classifies likely cause/action. | Verifier / evidence | Partial | Adopt | Command, diagnostic, failure kind/signature, source excerpt, candidates are carried. | Add assessment parser, diagnostic payload schema, assertion/import/test analysis, weak reason and weak target filters. |
...
| Repair patch executor/validation | `repair_patch_executor.rs`, `repair_patch_validation.rs`, `patch_provider.rs`, `patch_proposal.rs`, `repair_test_weakening_filter.rs` | Applies bounded patches, rejects unsafe/noop/duplicate/test-weakening patches, can rollback worsened patches. | Tools / minimal loop | Missing | Adopt | Model edits through normal tools; no dedicated patch validation/rollback state. | Add patch validation and rollback only within explicit repair job state. |
...
5. Semantic failure data is mostly labels and candidate lists, not clusters.
