...
| C24 | Repair attempt ledger | `repair_attempt_outcome.rs`, `repair_progress.rs`, `repair_job.rs` | Records attempt outcome by target and failure cluster. | Recovery task / repair loop | Partial | Adopt | Repair loop records concrete `passed`, `noop`, `malformed`, `unsafe`, `duplicate`, `no_progress`, `improved_still_failing`, `worsened`, and `explicit_stop` outcomes with target, role, cluster, changed files, and signatures. | Add broader focused live eval coverage across all profile families. |
...
| C26 | Verifier diagnostic assessment | `verifier_diagnostic_flow.rs`, `verifier_diagnostic_attempt.rs`, `verifier_diagnostic_prompt.rs`, `verifier_assessment_parser.rs`, `verifier_weak_reason.rs`, `verifier_weak_repair_target.rs` | Parses verifier failure and classifies likely cause/action. | Verifier / evidence | Partial | Adopt | Command, diagnostic, failure kind/signature, source excerpt, candidates, diagnostic payload schema, initial Python/FastAPI/Rust/Next.js/common diagnostic codes, observed/expected pairs, affected cases, preferred repair role, weak source-grep/self-referential verifier reasons, command-not-found, port-in-use, and admitted cluster targets are carried. Eval now reports unknown diagnostic count. | Add deeper language-specific assessment, richer weak target filters, and verifier attempt flow parity. |
...
| C32 | Repair patch executor/validation | `repair_patch_executor.rs`, `repair_patch_validation.rs`, `patch_provider.rs`, `patch_proposal.rs`, `repair_test_weakening_filter.rs` | Applies bounded patches, rejects unsafe/noop/duplicate/test-weakening patches, can rollback worsened patches. | PatchValidation Contract / repair loop / attempt ledger | Partial | Adopt | Phase 12 adds a common `PatchProposal` / `PatchValidationReport` boundary, rejects test weakening, out-of-scope paths, protected inputs, generated/build outputs, and dependency-cache mutations before progress is claimed, and reports patch validation through ContractEvidence/eval fields. | Direct patch executor parity and verifier-proven rollback execution remain partial; rollback is currently an explicit admission gate with rejected/admitted evidence. |
