...
pub fn guidance(goal: &str) -> String {
    ...
    format!(
        "For the nextjs profile, create a runnable Next.js app, not only package metadata. \
         ...
         For interactive UI, expose task-agnostic observability hooks: data-anvil-action=\"primary\" on the main start/submit/action control, and data-anvil-state containing JSON for meaningful visible state after each render. When the contract includes start_or_restart_flow, also expose data-anvil-action=\"restart\" on a reachable control that resets observable state.{port}"
    )
}
...
- Keep TypeScript and app router configuration coherent.\n\
- Keep a single route-bound implementation; do not leave capability components unimported.\n\
- For interactive UI, preserve or add data-anvil-action=\"primary\" on the main start/submit/action control and data-anvil-state with a JSON snapshot of meaningful visible state. When the contract includes start_or_restart_flow, preserve or add data-anvil-action=\"restart\" on a reachable control that resets observable state.\n\
- Do not treat scaffold-only, package-only, or build-only output as complete."
        ),
...
- Keep a single route-bound implementation; do not leave capability components unimported.\n\
- For interactive UI, expose data-anvil-action=\"primary\" on the main start/submit/action control and data-anvil-state with a JSON snapshot of meaningful visible state. When the contract includes start_or_restart_flow, expose data-anvil-action=\"restart\" on a reachable control that resets observable state.\n\
- Do not treat scaffold-only, package-only, or build-only output as complete."
        ),
    }
}
