...
pub(crate) fn compile_repair_prompt_section_with_root(
    ...
    let repairable = crate::minimal_loop::compile_repair_scope::repairable(root, errors);
    let mut lines = errors
        .iter()
        .filter(|error| crate::minimal_loop::compile_repair_scope::is_foreign(root, error))
        .map(|error| {
            format!(
                "Foreign compile error outside the selected route: {}. Do not edit this file automatically.",
                error.location()
            )
        })
        .collect::<Vec<_>>();
    if repairable.is_empty() {
        lines.push(
            "No route-bound compile repair target is authorized; stop with foreign_project_compile_error."
                .to_string(),
        );
        return lines.join("\n");
    }
    lines.extend(compile_error_repair_guidance_with_root(root, &repairable));
    let paths = repairable
        .iter()
...
export default function SpaceInvaders() {\n\
  ...
  return <main>{playerX}</main>;\n\
    ...
    #[test]
    fn compile_repair_prompt_refuses_foreign_unbound_source_edits() {
        ...
        assert!(prompt.contains("foreign_project_compile_error"), "{prompt}");
        assert!(
            prompt.contains("Do not edit this file automatically"),
            "{prompt}"
        );
        assert!(!prompt.contains("You MUST modify"), "{prompt}");
