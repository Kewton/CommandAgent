...
export default function SpaceInvaders(){ return CANVAS_W; }"#,
    ...
    #[test]
    fn route_bound_closure_includes_nextjs_api_routes_and_imports() {
        let dir = tempfile::tempdir().unwrap();
        ...
        std::fs::write(
            ...
            r#"import { writeTasks } from "@/lib/tasks"; export async function POST(){ await writeTasks([]); return Response.json([]); }"#,
        )
        .unwrap();
        std::fs::write(
            dir.path().join("src/lib/tasks.ts"),
            r#"import { writeFile } from "fs/promises"; export async function writeTasks(tasks: unknown[]){ await writeFile("tasks.json", JSON.stringify(tasks)); }"#,
        )
        .unwrap();

        let closure = route_bound_closure(dir.path(), "nextjs");

        assert!(closure.contains(Path::new("src/app/api/tasks/route.ts")));
        assert!(closure.contains(Path::new("src/lib/tasks.ts")));
    }
