...
| reason | 対象 |
|---|---|
| `current_worktree` | 実行中の worktree（`git rev-parse --show-toplevel` と一致するもの） |
| `main_worktree` | porcelain metadata から識別した primary/main worktree。current や integration と別でも保護する |
...
`current_worktree` と `main_worktree` と `integration_worktree` は、issue 番号や path で明示指定されても
...
- CLI に sync 相当が無い / 失敗した → `commandmate_sync.outcome` を `unavailable` / `failed` とし、
  `ids_reported: false`、`worktree_ids: []` とする。**run 全体を failure にしない。**
- CLI が件数だけ返した → `outcome: synced`、`reported_removed_count` に CLI の値、
  `ids_reported: false`、`worktree_ids: []` とする。
