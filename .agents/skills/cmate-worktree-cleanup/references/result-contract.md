# plan / result contract v2

`cmate-worktree-cleanup` が返す2つの文書を定義する。機械検証用の正本は
[../schemas/cleanup-plan.v2.json](../schemas/cleanup-plan.v2.json) と
[../schemas/cleanup-result.v2.json](../schemas/cleanup-result.v2.json) であり、
この文書は読み方と、schema では表現できない規則を述べる。

`plan_schema_version` / `result_schema_version` はともに 2 である。field の追加・削除・
...
- `excluded` — current / main / integration / 対象外。`reason` を必ず付ける。main worktree は current や integration と一致しなくても `main_worktree` として指定されても消せない。
...
- `evidence.verified_at` は対象ごとに drift 再検査が成功した直後、削除前に取得する。同じ run の最後に作る receipt timestamp を全対象へ流用しない。
- `skipped` は残した worktree のみ。dirty / detached / unmerged / unverifiable / excluded /
  plan_drift はすべてここ。dry-run で plan 上 `decision: delete` の対象も、まだ消していないため
  `reason: pending_confirmation` と元の `proof_type` でここへ置く。`reason` と `proof_type`
  （`excluded` を含む）を付ける。
...
`outcome` は `synced` / `unavailable` / `failed` / `skipped`。CLI が worktree id を明示したかを
`ids_reported`、件数を明示したかを `reported_removed_count` に別々に記録する。件数だけ返る
成功は `outcome: synced`、`ids_reported: false`、`worktree_ids: []` とする。件数から id を推測せず、
id 配列の長さから reported count を捏造しない。`unavailable` / `failed` でも未知 id を書かない。
...
| id | 内容 |
|---|---|
| `exclusions_honored` | current / main / integration worktree を一切消していない |
...
- field / enum 値の追加・削除・意味変更 → schema version を上げる。v1 文書は履歴証跡として v1 schema で読み、書き換えない。
