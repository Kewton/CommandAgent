...
2. **branch/base/path/baseline を hardcode しない。** `develop` / `feature/...` / npm / Cargo を
   決め打ちせず、[`references/profiles.md`](./references/profiles.md) に従って git metadata と `profile` から解決する。検証済み profile は Node/CommandMate と
...
2. `profile` を解決し、`base`（例 `origin/main`）・`remote`・`baseline`・main worktree・integration worktree・
...
2. current worktree・main worktree・integration worktree・対象外 issue を `excluded` に振り分ける
   （[references/safety.md](./references/safety.md) §2）。**この3つは指定されても消さない。**
...
1. plan（[references/result-contract.md](./references/result-contract.md) §1）を dry-run で提示する。
   ...
   dry-run result では、plan 上 deletable な対象も未削除なので `skipped` に
   `reason: pending_confirmation` と元の proof type を記録する。
...
1. **drift 再検査**: 削除の直前に各対象の status / tip / ref を再取得し、plan と照合する。
   ...
   成功した対象は、その対象の再検査直後・削除前に `evidence.verified_at` を取得する。
   run 終了時の timestamp を全対象へ流用しない。
...
1. `commandmate` public CLI で sync する（§5）。無い/失敗は `unavailable`/`failed` とし、
   `ids_reported: false` と空の `worktree_ids` で返す。**run を failure にしない。**
2. server / process / tmux / DB / log は §6 に従い診断表示のみ。`next_actions` に回す。

### Step 8 — result と summary

`result` 文書（`schemas/cleanup-result.v2.json` 適合、`result_schema_version: 2`）と
...
CLI に sync が無い/失敗した場合は `commandmate_sync.outcome` を `unavailable`/`failed` にして
run を失敗にしない。CLI が件数だけ返した場合は `synced` のまま `ids_reported: false`、
`reported_removed_count: <CLIの値>`、`worktree_ids: []` とし、id を推測しない。
...
| sync 不可 | `unavailable`/`failed`。`ids_reported: false` と空の id 配列。run は failure にしない |
...
1. `exclusions_honored` — current / main / integration worktree を一切消していない。
...
- [`references/profiles.md`](./references/profiles.md) — profile の discovery と commandagent / commandmate 解決規則
- [`references/agent-compatibility.md`](./references/agent-compatibility.md) — Agent 差異と fallback
- [`schemas/cleanup-plan.v2.json`](./schemas/cleanup-plan.v2.json) — 現行 plan 文書 schema
- [`schemas/cleanup-result.v2.json`](./schemas/cleanup-result.v2.json) — 現行 result 文書 schema
- `schemas/*.v1.json` — 履歴文書の読み取り専用 schema（新規出力には使わない）
