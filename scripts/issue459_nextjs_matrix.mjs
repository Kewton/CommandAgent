...
const integration = path.join(repo, 'tests/corpus/apps/issue459-create-recovery-integration');
...
const results = [];
for (const c of JSON.parse(fs.readFileSync(path.join(integration, 'cases.json')))) {
  const root = path.join(work, c.name);
  fs.cpSync(original, root, { recursive: true, errorOnExist: true });
  for (const overlay of c.overlays) fs.cpSync(path.join(['ui-only', 'observable'].includes(overlay) ? integration : fixture, 'overlays', overlay), root, { recursive: true });
