...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Report A23 exploratory natural Recovery exposure pilot"
    )
    parser.add_argument(
    ...
    report = build_recovery_a23_pilot_report(
        records=records,
        contract=contract,
        oracle_executability_preflight=preflight,
    )
    _atomic_json(run_dir / "recovery-a23-pilot-report.json", report)
